...
function openRsmCustom() {
  ...
  body.innerHTML = available.map(name => `
    <label style="display:flex;align-items:center;gap:8px;padding:4px 0;cursor:pointer;font-size:0.78rem;color:#e2e8f0;">
      <input type="checkbox" value="${name}" ${current.includes(name)?'checked':''}>
      <span style="color:#e2e8f0;">${RSM_LABEL[name]||name}</span>
    </label>
  `).join('');
